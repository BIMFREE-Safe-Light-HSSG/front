...
export function collectAssets(doc: SceneGraphSkeleton): FacilityAssetRef[] {
  const zones = doc.scene_graph.nodes.filter((n): n is ZoneNode => n.type === "ZONE")
...
export function uniqueAssetClasses(assets: FacilityAssetRef[]): string[] {
  return [...new Set(assets.map((a) => a.class))].sort((a, b) => a.localeCompare(b, "ko"))
}

/** Placeholder maintenance rows until API is wired. */
