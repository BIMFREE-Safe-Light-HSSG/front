...
function ZonePanel({
  ...
  useEffect(() => {
    ...
    disableRaycast(outlineRef.current as unknown as THREE.Mesh);
    disableRaycast(glowRef.current as unknown as THREE.Mesh);
    return () => {
      geometry?.dispose();
      floorPick?.dispose();
      outline?.dispose();
  ...
  return (
    <group>
      ...
      {showMesh && selected && glowOutline ? (
        <line
          ref={glowRef}
          geometry={glowOutline}
