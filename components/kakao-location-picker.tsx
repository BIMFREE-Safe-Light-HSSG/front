...
const KAKAO_LOAD_HELP =
  "developers.kakao.com → 앱 → 제품 설정에서 「지도/로컬」 활성화, 플랫폼 → Web에 http://localhost:3000 등록, JavaScript 키 사용을 확인하세요.";

async function validateKakaoMapsKey(): Promise<void> {
  const response = await fetch("/api/kakao-map-check", { cache: "no-store" });
  const data = (await response.json()) as { ok?: boolean; message?: string };

  if (!data.ok) {
    throw new Error(data.message ?? `Kakao Maps SDK를 사용할 수 없습니다. ${KAKAO_LOAD_HELP}`);
  }
}

function loadKakaoMapsScript(appKey: string) {
  return new Promise<void>((resolve, reject) => {
    const finishLoad = () => {
      if (!window.kakao?.maps) {
        reject(new Error(`Kakao Maps SDK가 초기화되지 않았습니다. ${KAKAO_LOAD_HELP}`));
        return;
      }
    ...
    if (existingScript) {
      ...
      existingScript.addEventListener(
        "error",
        () => reject(new Error(`Kakao Maps SDK 스크립트 로드 실패. ${KAKAO_LOAD_HELP}`)),
        { once: true },
      );
      return;
    }
    ...
    script.onerror = () => reject(new Error(`Kakao Maps SDK 스크립트 로드 실패. ${KAKAO_LOAD_HELP}`));

    document.head.appendChild(script);
  });
}

async function loadKakaoMaps(appKey: string) {
  ...
  if (!window.__kakaoMapsLoadingPromise) {
    window.__kakaoMapsLoadingPromise = validateKakaoMapsKey()
      .then(() => loadKakaoMapsScript(appKey))
      .catch((error) => {
        delete window.__kakaoMapsLoadingPromise;
        throw error;
      });
  }
...
export function KakaoLocationPicker({
  ...
  useEffect(() => {
    ...
    const container = mapContainer ?? mapContainerRef.current;
    if (!container) return;
    ...
    loadKakaoMaps(appKey)
      .then(() => {
        if (!isMounted || !container || !window.kakao?.maps) return;
    ...
    <div className="space-y-3">
      <div className="flex gap-2">
        <input
          ...
          className={
            isGlass
              ? `h-10 min-w-0 flex-1 rounded-md border px-3 text-sm outline-none transition-[color,box-shadow] focus-visible:ring-[3px] ${glassFieldClass}`
              : "h-10 min-w-0 flex-1 rounded-md border border-input bg-background px-3 text-sm outline-none transition-[color,box-shadow] focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50"
          }
        />
        <button
          type="button"
          ...
          className={
            isGlass
              ? "h-10 rounded-full bg-red-950 px-4 text-xs font-bold tracking-wide text-white uppercase transition-colors hover:bg-black disabled:pointer-events-none disabled:opacity-50"
              : "h-10 rounded-md bg-red-950 px-4 text-sm font-medium text-white transition-colors hover:bg-black disabled:pointer-events-none disabled:opacity-50"
          }
        >
          {isSearching ? "검색 중" : "검색"}
        </button>
      ...
      <div
        className={
          isGlass
            ? "overflow-hidden rounded-xl border border-red-900/15 bg-white/40"
      ...
      >
        <div ref={(node) => {
          ...
          className="h-72 w-full" aria-label={label} />
      </div>
      <div
        className={`flex items-center justify-between gap-3 text-xs ${isGlass ? "text-zinc-700" : "text-muted-foreground"}`}
      >
        <span
          className={
            status === "error"
        ...
        <span className={`font-mono text-[10px] tracking-wide ${isGlass ? "text-zinc-600" : ""}`}>
